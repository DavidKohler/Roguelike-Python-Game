...
def cast_fireball(*args, **kwargs):
    ...
    results.append({'consumed': True, 'message': Message(
        'The fireball explodes, burning everything within {0} tiles!'.format(radius),
        libtcod.orange)})
...
def cast_lightning(*args, **kwargs):
    ...
    if target:
        ...
        results.append({'consumed': False, 'target': None, 'message': Message(
            'No enemy is close enough to strike.', libtcod.yellow)})
...
def cast_mistletoe(*args, **kwargs):
    ...
    if target:
        results.append({'consumed': True, 'target': target, 'message': Message(
            'A dart of mistletoe strikes the {0}! The damage is {1}'\
            .format(target.name, damage), libtcod.orange)})
        results.extend(target.fighter.take_damage(damage))
    else:
        results.append({'consumed': False, 'target': None, 'message': Message(
...
def cast_ragnarok(*args, **kwargs):
    ...
    results.append({'consumed': True, 'message': Message(
        'Ragnarok erupts, blasting everything within {0} tiles!'.format(radius),
        libtcod.orange)})
    ...
    for entity in entities:
        if entity.distance(target_x, target_y) <= radius \
            ...
            results.append({'message': Message(
                'The {0} gets blasted for {1} hit points.'.format(entity.name, damage),
                libtcod.orange)})
            results.extend(entity.fighter.take_damage(damage))
